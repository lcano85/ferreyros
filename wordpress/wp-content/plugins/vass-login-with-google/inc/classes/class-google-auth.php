...
class Google_Auth {
	...
	protected function _get_client() {
		...
		$redirect_to = ( ! empty( $redirect_to ) ) ? $redirect_to : home_url();
	...
	protected function _get_user_from_token( $token ) {
		...
		try {
			...
			//Obtener información de los Grupos relacionados al usuario logueado
			$clientAccountService = new \Google_Client(['subject'=> $group_key]);
			$clientAccountService->setAuthConfig($service_account);
			$clientAccountService->setScopes(array(
			    \Google_Service_Directory::ADMIN_DIRECTORY_GROUP_READONLY,
			    \Google_Service_Directory::ADMIN_DIRECTORY_GROUP_MEMBER_READONLY,
	           	\Google_Service_Directory::ADMIN_DIRECTORY_USER_READONLY,
			));
			$directoryservice = new \Google_Service_Directory( $clientAccountService );
			$optParams = array('userKey' => $google_userinfo['id'], 
				'domain' => $google_userinfo['hd']);
			$groupsList = $directoryservice->groups->listGroups($optParams);
			$groupsInfo = json_encode($groupsList->getGroups());
	...
	protected function _create_user( $user_info = [] ) {
		...
		if ( ! empty( $user_id ) && ! is_wp_error( $user_id ) ) {
			add_user_meta( $user_id, 'vass_groups_json', $user_info['groups_json']);
			return $user_id;
		} else{
			return 0;
		}
	...
	/**
	* Verification of User Groups and Roles
	*/
	protected function _verifyGroupsRoles($blog_id, $user, $user_info = []) {
		$roles = wp_roles()->roles;
		$userGroups = json_decode($user_info['groups_json']);
		foreach( $userGroups as $userGroup ) {
			$existRol = false;
			foreach ( $roles as $key => $value ){
				if(strcmp($userGroup->id, $key) == 0){
					$existRol = true;
					break;
				}
			}
			if(!$existRol){
				add_role( $userGroup->id, $userGroup->name, array(
					'read' => true,
				));
			}
			$nameMeta = 'wp_capabilities';
			if($blog_id > 1){
				$nameMeta = 'wp_'.$blog_id.'_capabilities';
			}
			$curArrayRol = get_user_meta($user->ID, $nameMeta, true);
			$curArrayRol[$userGroup->id] = true;
			update_user_meta( $user->ID, $nameMeta, $curArrayRol );
		}
		return true;
	}
	...
	public function authenticate_user( $user = null ) {
		...
		if ( ! empty( $user ) && $user instanceof \WP_User ) {
			//Update Groups JSON
			update_user_meta( $user->ID, 'vass_groups_json', $user_info['groups_json'] );
		...
		if ( ! $this->_can_users_register() ) {
			return new \WP_Error(
				...
				sprintf( __( 'User <strong>%s</strong> not registered in WordPress.', 'vass-login-with-google' ), $user_info['user_email'] )
			);
		}
		...
		if ( ! $this->_can_register_with_email( $user_info['user_email'] ) ) {
			return new \WP_Error(
				'wp_google_login_error',
				// translators: %s: User email.
				sprintf( __( 'User can not register with <strong>%s</strong> email address.', 'vass-login-with-google' ), $user_info['user_email'] )
			);
		}
		...
		if ( $is_mu_site ) {
			...
			//Verify Groups - Roles from User
			$this->_verifyGroupsRoles($blog_id, $user, $user_info);
		}
