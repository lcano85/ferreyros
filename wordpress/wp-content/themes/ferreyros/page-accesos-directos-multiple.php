<?php
get_header();
/* Template Name: Página Accesos directos multiple*/
global $post; 
...
<?php $hero = get_field('banner'); ?>
    ...
    <?php 
        $accesos_multiples = get_field('accesos_multiples'); 
        if( $accesos_multiples ) :
    ?>
    <section class="ferreyros__page--content fred__page--<?php echo $post_slug = $post->post_name; ?> my-5">
        <?php $row_list = $accesos_multiples['lista']; ?>
        <div class="container">
            <?php foreach( $row_list as $row_item ): ?>
            <div class="row <?php echo $post_slug = $post->post_name; ?>__list my-5">
                <div class="ferreyros__page--header mb-4">
                    <h3 class="title m-0"><?php echo $row_item['titulo'] ?></h3>
                </div>
                <?php $rows = $row_item['accesos_directos'] ?>
